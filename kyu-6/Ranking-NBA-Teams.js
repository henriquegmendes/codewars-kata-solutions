...
function nbaCup(resultSheet, toFind) {
  ...
  let totalDraws = 0;
  let totalLosses = 0;
  if (teamsArr.indexOf(toFind) === -1) {
    return `${toFind}:This team didn't play!`;
  }
  for (let i = 0; i < resultsArr.length; i += 1) {
    if (resultsArr[i].includes(toFind)) {
      let pointsScored = 0;
      let pointsConceded = 0;
      
      switch (pointsScored > pointsConceded) {
        case true:
  ...
  return `${toFind}:W=${totalWins};D=${totalDraws};L=${totalLosses};Scored=${totalPointsScored};Conceded=${totalPointsConceded};Points=${points}`;
}
