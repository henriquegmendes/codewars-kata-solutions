...
function isSumOfCubes(s) {
  const result = [];
  const array = s.split(' ').map(el => parseInt(el, 10)).forEach((element) => {
    if (element >= 0) {
      if (element.toString().length <= 3) {
        const sumCubes = element.toString().split('').map((e => e ** 3)).reduce((acc, elem) => acc + elem, 0);
        if (sumCubes === element) {
          result.push(element);
        }
      }
...
console.log(isSumOfCubes('111 9026315 -827&()'));
